...
3. **Playing state is read directly, never inferred** from notification arrival or ordering. ~~`playbackRate` is the source of truth for playing state.~~ **Superseded during task 6:** `bridge.m` calls `MRMediaRemoteGetNowPlayingApplicationIsPlaying` instead, because the rate field measured *inverted* for Spotify on macOS 26.6.2 — `1` while PAUSED, absent while PLAYING, correct in 1 of 5 samples. ✅ **The replacement is confirmed live (2026-08-30):** the author ran the signed bundle on real hardware and saw the ambient now-playing badge appear with playback and go away without it, and the badge draws only while `isPlaying` is true — so the flag is verified end to end, through the signed bundle, the helper, the bridge, the wire decode and the publish path. ⚠️ **The inversion finding stays on record and is still only observed** — one player, one machine, one session, no explanation — and confirming the replacement does not re-test the rate field. The rate remains a fallback; re-measure before ever promoting it again (see the spec, section 6). The reason the rule exists is unchanged; only the field changed.
...
- ~~`playing` comes from `kMRMediaRemoteNowPlayingInfoPlaybackRate > 0`.~~ **Superseded during this task:** `playing` comes from `MRMediaRemoteGetNowPlayingApplicationIsPlaying`. Measured by hand against Spotify on macOS 26.6.2, the rate field was inverted — published as `1` while PAUSED and absent while PLAYING, correct in 1 of 5 samples — and a rate that lies would draw a pause button over paused music. ✅ **The replacement is confirmed live (2026-08-30):** running the signed bundle on real hardware, the author saw the ambient now-playing badge appear beside the notch with playback and go away without it. The badge draws only while `isPlaying` is true, so this exercises the flag end to end — signed bundle, helper, bridge, `playing` on the wire, decode, publish. That is the live check this entry was pending. ⚠️ **The inversion itself remains observed, not proven,** and is kept here deliberately: one player, one machine, one session, no explanation, not independently confirmed — and the confirmation above does not re-test the rate field. The rate stays a fallback only; re-measure before promoting it back, and expect the inversion until a measurement says otherwise. Open but not blocking: a non-Spotify client and a second machine.
