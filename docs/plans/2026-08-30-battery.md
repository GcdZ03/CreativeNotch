...
|---|---|
| `accept` returns `estimate` immediately when non-nil | `aSingleReadingIsNotTrusted`, `theRoadmapsSwingIsRejected`, `nothingIsShownInsideTheSettlingWindow` |
| `isSettling` returns `false` unconditionally | `nothingIsShownInsideTheSettlingWindow`, `evenPerfectAgreementIsSuppressedInsideTheWindow`, `aSecondTransitionRestartsTheWindow` |
...
| The `guard let estimate` branch returns `nil` without clearing `candidate` | `anUnknownReadingBreaksTheAgreementRun` |
| `isSettling` uses `abs(now - lastTransition)` | `aBackwardClockDoesNotOpenTheGate` |
| The `defer` is removed | `aRejectedReadingBecomesTheNextCandidate` |
